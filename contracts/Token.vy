...
@dev Emits when ownership of any NFT changes by any mechanism.
     ...
     destroyed (`to` == 0). Exception: during contract creation,
     any number of NFTs may be created and assigned without emitting
     Transfer. At the time of any transfer, the approved address for
     that NFT (if any) is reset to none.
@param _from Sender of NFT
     (if address is zero address it indicates token creation).
...
@param _approved Status of operator rights
     (true if operator rights are given and false if revoked).
"""
...
def _transferFrom(
    ...
    @dev Exeute transfer of a NFT.
         ...
         Throws if `_from` is not the current owner.
         Throws if `_tokenId` is not a valid NFT.
    @notice `msg.sender` not allowed in private function so pass `_sender`
    """
    # Check requirements
...
def transferFrom(_from: address, _to: address, _tokenId: uint256):
    ...
    @dev Throws unless `msg.sender` is the current owner, an authorized
         ...
         Throws if `_to` is the zero address.
         Throws if `_tokenId` is not a valid NFT.
    @notice The caller is responsible to confirm that `_to` is capable
            of receiving NFTs or else they maybe be permanently lost.
...
def safeTransferFrom(
    ...
    @dev Transfers the ownership of an NFT from one address to another address.
         ...
         Throws if `_to` is the zero address.
         Throws if `_tokenId` is not a valid NFT.
         If `_to` is a smart contract, it calls `onERC721Received` on `_to`
    ...
    @param _to The new owner.
    @param _tokenId The NFT to transfer.
    @param _data Additional data with no specified format, forwarded to `_to`.
    """
    self._transferFrom(_from, _to, _tokenId, msg.sender)
    if(_to.codesize > 0):  # check if `_to` is a contract address
        returnValue: bytes32 = ERC721Receiver(_to).\
...
def approve(_approved: address, _tokenId: uint256):
    ...
    @dev Set or reaffirm the approved address for an NFT.
         ...
         Throws if `_tokenId` is not a valid NFT.
         (NOTE: This is not written the EIP)
...
def setApprovalForAll(_operator: address, _approved: bool):
    ...
    @dev Enables or disables approval for a third party ("operator")
         to manage all of `msg.sender`'s assets. It also emits the
         ApprovalForAll event.
         Throws if `_operator` is the `msg.sender`.
         (NOTE: This is not written the EIP)
    @notice This works even if sender doesn't own any tokens at the time.
    @param _operator Address to add to the set of authorized operators.
    @param _isApproved True if the operators is approved,
        false to revoke approval.
    """
    # Throws if `_operator` is the `msg.sender`
